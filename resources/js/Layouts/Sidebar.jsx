    ...
    return (
      <>
        ...
        <button
          ...
          className={`fixed top-1/2 -translate-y-1/2 bg-[#10121f] text-white p-2 hover:bg-[#405ecf] z-50 shadow-lg border-r border-t border-b border-white transition-all duration-300 ease-in-out ${
        ...
        <aside
          ref={sidebarRef}
          className={`fixed left-0 top-0 h-full w-64 bg-[#10121f] shadow-2xl text-gray-100 z-50 transform transition-transform duration-300 ease-in-out flex flex-col ${
            isMobileOpen ? "translate-x-0" : "-translate-x-full"
          }`}
        >
          {/* Header */}
          <div className="px-4 py-6 border-b border-[#6b87ee]">
            <p className="text-lg text-gray-200 mt-1">Credit Cards App</p>
          </div>
              ...
              <li key={idx} onClick={() => handleNavigate(item.path, item.comingSoon)}>
                ...
                >
                  <span className="pointer-events-none relative">
                    ...
                    {/* Badge Coming Soon - Mobile (updated to match desktop) */}
                    {item.comingSoon && (
                      <span className="absolute -top-1 -right-1 w-2.5 h-2.5 bg-[#9843E9] rounded-full"></span>
                    )}
                  </span>
                  <span className="text-sm font-medium pointer-events-none">{item.label}</span>

                  {/* Badge Notifikasi - Mobile (updated to match desktop) */}
                  ...
                  {item.comingSoon && (
                    <span className="ml-auto text-xs bg-[#9843E9] text-white px-2 py-0.5 rounded-full font-medium">
                      Coming Soon
                    </span>
                  )}
                </div>
  ...
  return (
    <>
      <aside
        ...
        className={`flex flex-col transition-all duration-300 bg-[#10121f] shadow-lg text-gray-100 h-full ${
            ...
            <li key={idx} onClick={() => handleNavigate(item.path, item.comingSoon)}>
              <div
                className={`flex items-center gap-3 rounded-lg px-3 py-3 transition-all cursor-pointer relative
                  hover:text-white hover:bg-[#9843E9] ${
                    url === item.path
                      ? "bg-[#9843E9] text-white font-semibold"
                      : "text-gray-100"
                  }`}
              >
